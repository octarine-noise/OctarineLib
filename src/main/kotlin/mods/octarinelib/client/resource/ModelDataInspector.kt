package mods.octarinelib.client.resource

import mods.octarinelib.common.config.BlockMatcher
import mods.octarinelib.loader.RefsCore
...
abstract class ModelDataInspector {
    ...
    fun handleLoadModelData(event: LoadModelDataEvent) {
        ...
        val stateModels = RefsCore.stateModels.get(event.loader) as Map<ModelResourceLocation, IModel>
...
val IModel.modelBlockAndLoc: Pair<ModelBlock, ResourceLocation>? get() {
    if (RefsCore.VanillaModelWrapper.isInstance(this))
        return Pair(RefsCore.model_VMW.get(this) as ModelBlock, RefsCore.location_VMW.get(this) as ResourceLocation)
    else if (RefsCore.WeightedPartWrapper.isInstance(this)) RefsCore.model_WPW.get(this)?.let {
        return (it as IModel).modelBlockAndLoc
    }
    else if (RefsCore.WeightedRandomModel.isInstance(this)) RefsCore.models_WRM.get(this)?.let {
